...
def app(data):
    ...
    def comp_feature_information_gain(data, target, descriptive_feature, split_criterion):
        ...
        for level in data[descriptive_feature].unique():
            ...
            entropy_level = compute_impurity(
                data_feature_level[target], split_criterion)
    ...
    if st.button("Information gain"):
        ...
        for feature in data.drop(columns=classatr).columns:
            feature_info_gain = comp_feature_information_gain(
                data, classatr, feature, split_criterion)
    ...
    if st.button("Gini Index"):
        ...
        for feature in data.drop(columns=classatr).columns:
            feature_info_gain = comp_feature_information_gain(
                data, classatr, feature, split_criteria)
            if feature_info_gain > mx:
                mx = feature_info_gain
                ftr = feature
    ...
    def DeciTree():
        ...
        st.write('Misclassification Rate : ', 1 -
        ...
        st.write("Recall(Sensitivity): ", recall_score(
            y_test, y_pred, average="macro"))
        st.write("Specificity: ", recall_score(
            y_test, y_pred, pos_label=0, average="macro"))
